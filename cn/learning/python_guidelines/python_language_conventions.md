...
translate_url: /learning/python_guidelines/python_style_conventions.htmles/python_language_conventions.html
mathjax: false
---

## Lint

> 💡 **提示**: 用 [pylintrc](https://google.github.io/styleguide/pylintrc) 运行 pylint, 以检查你的代码.
...
> 💡 **提示**: 使用 `import` 语句时, 只导入包和模块, 而不单独导入函数或者类。

**定义**:
用于方便模块间共享代码的重用机制.

**优点**:
命名空间的管理规范十分简单. 每个标识符的来源对用一致的方式来表示. `x.Obj` 表示 `Obj` 对象定义在模块 `x` 中.
...
2. [six.moves](https://six.readthedocs.io/#module-six.moves) 模块中的重定向.

## 包

> 💡 **提示**: 使用每个模块的完整路径名来导入模块.
...
3. 永远不要使用 `except:` 语句来捕获所有异常, 也不要捕获 `Exception` 或者 `StandardError` , 除非你想:
    ...
    如果你使用这种写法, Python 将非常宽容. `except:` 真的很会捕获任何错误, 包括拼写错误的符号名、 `sys.exit()` 调用、 `Ctrl+C` 中断、单元测试错误和各种你不想捕获的错误.
...
> 💡 **提示**: 只要可行, 就用列表、字典和文件等类型的默认迭代器和操作符.
...
**优点**:
代码简单, 因为生成器可以保存局部变量 and 控制流. 相较于直接创建整个列表的函数, 生成器使用的内存更少.
...
> 💡 **提示**: 适用于单行函数. 建议用生成式替代 `map()/filter()` 与 `lambda` 的组合.
...
**优点**:
很多时候, 你需要一个拥有大量默认值的函数, 并且偶尔需要覆盖 these 默认值. 通过默认参数值可以轻松实现这种功能, 不需要为了覆盖默认值而编写大量额外的函数. 同时, Python不支持重载方法和函数, 而默认参数的写法可以轻松"仿造"重载行为.
...
> 💡 **提示**: 可以用特性来读取或设置涉及简单计算、逻辑的属性. 特性的实现必须和属性 (attribute) 一样满足这些通用要求: 轻量、直白、明确.
...
**结论**:
允许使用特性. 但是, 和运算符重载一样, 只能在必要时使用, 并且要模仿常规属性的存取特点. 若无法满足要求, 请参考相关规则.

举个例子, 一个特性不能仅仅用于获取和设置一个内部属性: 因为不涉及计算, 没有必要用特性 (应该把该属性设为公有). 而用特性来限制属性的访问或者计算 **简单** 的衍生值则是正确的: 这种逻辑简单明了.

应该用 `@property` 装饰器来创建特性. 自行实现的特性装饰器属于威力过大的功能.
...
> 💡 **提示**: 仅在有显著优势时, 审慎地使用装饰器. 避免使用 `staticmethod`. 减少使用 `classmethod`.

**定义**:
装饰器(也就是@标记)作用在函数和方法上. 常见的装饰器是 `@property`, 用于把方法转化为动态求值的属性. 不过, 也可以用装饰器语法自行定义装饰器. 
...
> 💡 **提示**: 可以通过导入 `__future__` 包, 在较老的运行时上启用新语法, 并且只在特定文件上生效.
...
详情参见 [Python future 语句](https://docs.google.com/3/library/__future__.html) 的文档.


## 代码类型注释

> 💡 **提示**: 你可以根据 [PEP-484](https://www.python.org/dev/peps/pep-0484/) 来对 python3 代码进行注释.
...
**结论**:
强烈推荐你在更新代码时启用 python 类型分析. 在添加或修改公开API时, 请添加类型注释, 并在构建系统中启用 pytype.
